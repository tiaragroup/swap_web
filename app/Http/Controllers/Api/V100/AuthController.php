...
class AuthController extends Controller
{
    ...
    {
        try {
                $validator = Validator::make($request->all(), ['phone' => 'required'], ['phone.required' => 'The phone number field is required.']);
                if ($validator->fails()) { return $this->responseWithError(__('Required field missing'), $validator->errors(), 422); }
                $user = User::where('phone', $request->phone)->where('user_type','customer')->where('is_deleted',0)->first();
                if($user){
                    $token = $user->createToken('SwapApptCustomerAuth')->plainTextToken;
                    if($user->is_user_banned == 1) { return $this->responseWithError( __('Your account has been banned'), [], 401); }
                    if ($user->status == 0):
                    ...
                    if (!$user->is_phone_verified && !$user->is_completed_profile) {
                        
                        return response()->json(['token' => $token, 'is_phone_verified' => $user->is_phone_verified,'is_completed_profile' => $user->is_completed_profile], 200);
                    }
                    ...
                    $req = DB::table('phone_verifications')->where('phone',$request->phone)->first();
                    if ($req && Carbon::parse($req->created_at)->addMinutes(2) >= Carbon::now()){ return $this->responseWithError(__('Verification Code was Already Sent'), $validator->errors(), 500); }
                    // start implement otp
                    $otp = rand(1000, 9999);
                    ...
                    if ($response != 'success') {
                        return response()->json(['error' => __('Unable to send otp')], 405);
                    }
                    // end implement otp
                    DB::commit();
        ...
        try{
            ...
            if ($user) {
                if ($user->is_phone_verified) {
                    return response()->json(['message' => 'phone_number_is_already_varified' ], 200);
                }
                ...
                if ($data) {
                    ...
                    return response()->json([
                        'message' => 'phone_number_varified_successfully',
                        'otp' => 'inactive'
                    ], 200);
                    
                } else {
                    return $this->responseWithError('phone_number_and_otp_not_matched',[],404);
                }
